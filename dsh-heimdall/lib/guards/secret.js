...
function extractDecodedText(output) {
...
function containsSecretInHex(output, secretValues) {
...
export function redactOutput(output, secretValues) {
    ...
    result = result.replace(/(\b\w*(?:SECRET|KEY|TOKEN|PASSWORD|PASS|APIKEY|CREDENTIAL|PRIVATE)=)\S*/gi, `$1${REDACTED}`);
    return result;
}
/**
 ...
 */
export function loadSecretGuardState(dotenvPath) {
    const state = {
        secretKeys: [],
        secretValues: {},
        keyPattern: null,
    ...
    try {
        if (!dotenvPath)
            return state;
        parsed = JSON.parse(readFileSync(dotenvPath, 'utf8'));
    }
    catch {
        return state;
    }
...
export function getSecretReference(command, state) {
    if (!state.keyPattern)
        return null;
    const match = command.match(state.keyPattern);
    return match?.[0] ?? null;
}
export function getSecretGuardBlockReason(secretName) {
    return (`Blocked: command references secret "${secretName}". ` +

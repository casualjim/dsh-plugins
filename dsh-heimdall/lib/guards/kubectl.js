 ...
 */
const SEG = '(?:[^;|&\\n]|\\\\\\n)';
export const KUBECTL_BLOCKED = new RegExp(`\\bkubectl\\b${SEG}*\\b(?:(get\\b${SEG}*\\bsecrets?\\b)|(patch\\b${SEG}*finalizers)|(exec\\b${SEG}*(?:app\\.ini|/var/run/secrets|\\bprintenv\\b|\\benv\\b)))`);
export function getKubectlBlockReason(command) {
    const m = KUBECTL_BLOCKED.exec(command);
    if (!m)
        return null;
    const trailer = `This is protected by dsh-heimdall/kubectl-secret-guard. ` +
    ...
    if (m[2]) {
        return (`Blocked: command would patch finalizers. ` +
            `Removing finalizers bypasses Kubernetes deletion safeguards and can cause resource leaks. ` +
            trailer);
    }
    return (`Blocked: command would exec into a pod and access sensitive data ` +

...
function matchWrapperAt(tokens, pos) {
    let best = 0;
    for (const seq of WRAPPER_COMMANDS) {
        if (seq.length <= best)
            continue;
        if (seq.every((t, i) => tokens[pos + i] === t))
            best = seq.length;
    }
    return best;
}
...
function isOp(t, op) {
    return typeof t === 'object' && t !== null && 'op' in t && t.op === op;
}
function opOf(t) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (isOp(t, '<') && i + 1 < tokens.length && isOp(tokens[i + 1], '<')) {
            ...
            if (i + 1 < tokens.length && isStringToken(tokens[i + 1])) {
                i++;
                heredocDelim = tokens[i].replace(/^['"]|['"]$/g, '');
            }
            continue;
        }
        const op = opOf(t);
        if (op !== undefined && COMMAND_SEPARATORS.has(op)) {
            if (op === '|') {
                pipedOut = true;
                close();
            }
        ...
        if (op !== undefined && REDIRECT_OPS.has(op)) {
...
function matchSegment(segment, policies, checkRecursive, inheritedNonBare) {
    ...
    for (const policy of policies) {
        ...
        if (bl.length === 0 || effective.length < bl.length)
            continue;
 ...
 */
export function splitShellLines(command) {
    const META_CHARS = ' \t|&;()<>';
    let out = '';
    let i = 0;
...
export function checkCommand(command, policies, nonBare = false) {
    const parsed = shellParse(splitShellLines(command));
    const segments = splitCommandSegments(parsed);
    const check = (cmd, inheritNonBare) => checkCommand(cmd, policies, inheritNonBare);
...
export function getCommandPolicyBlockReason(policy) {
    return (`Blocked: command violates repo policy "${policy.name}".\n` +
        `${policy.message}\n` +
        `This is protected by dsh-heimdall/command-policy-guard. ` +

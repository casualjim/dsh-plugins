...
const NO_SAFE_AHEAD = `(?!${SEG}*\\b(?:encrypt|rotate|publish|keyservice|filestatus|groups|updatekeys|set|unset|completion|help|h)\\b)`
    ...
    + `(?!${SEG}*(?:--version\\b|-v\\b))`;
export const SOPS_DECRYPT = new RegExp(`${SOPS_CMD}${SEG}*\\b(?:decrypt|exec-env|exec-file|edit)\\b`
    + `|${SOPS_CMD}${SEG}*(?:--decrypt\\b|-d\\b)`
    + `|${SOPS_CMD}${NO_SAFE_AHEAD}`, 'm');
export function getSopsBlockReason(command) {
    if (!SOPS_DECRYPT.test(command))
        return null;
    return (`Blocked: command would decrypt secrets via sops. ` +

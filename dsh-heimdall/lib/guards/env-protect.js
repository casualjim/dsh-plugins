...
function isExampleVariant(name) {
    const lower = name.toLowerCase();
    return EXAMPLE_SUFFIXES.some((suffix) => lower.endsWith(`.${suffix}`) || lower.includes(`.${suffix}.`));
}
export function isDotenvPath(rawPath) {
    const path = rawPath.replace(/^@/, '');
    const name = basename(path).toLowerCase();
    if (name === '.env' || name === '.envrc')
        return true;
    if (name.startsWith('.env.'))
        return !isExampleVariant(name);
    if (name.endsWith('.env'))
        return !isExampleVariant(name);
    return false;
}
export function getEnvProtectReason(path) {
    return (`Blocked: reading dotenv file "${path}" is forbidden. ` +

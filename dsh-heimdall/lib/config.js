 ...
 */
export function mergeSandboxOptions(...layers) {
    ...
    const empty = (list) => list === undefined || list.length === 0;
    const defined = (layer) => {
...
export function deepMerge(base, overrides) {
    ...
    for (const key of Object.keys(overrides)) {
        ...
        switch (key) {
            ...
            default: {
                if (key === 'dotenv')
                    result.dotenv = overrides.dotenv;
            }
        }
    }
    return result;
 ...
 */
function loadConfigLevel(dir, basename) {
...
function legacyUserConfigFiles() {
    const home = homedir();
    const files = [];
...
function legacyProjectConfigFiles(workspaceRoot) {
    const files = [];
 ...
 */
function migrate(legacyFiles, targetDir, targetBasename) {
    ...
    if (parseable.length === 0)
        return undefined; // nothing to migrate
    let merged = null;
    ...
    for (const file of parseable) {
        try {
            ...
            if (err.code !== 'ENOENT') {
                throw new Error(`dsh-heimdall: failed to remove migrated config ${file}: ${err.message}`);
            }
        }
    }
    return targetPath;
...
export function migrateUserConfig(configDir = heimdallConfigDir(), migrationErrors = []) {
...
export function migrateWorkspaceConfig(workspaceRoot, migrationErrors = []) {
...
export function loadConfig(workspaceRoot, rowConfig) {
    ...
    for (const message of migrationErrors)
        console.warn(`dsh-heimdall: ${message}`);
...
function ensureGeneratedDefaultConfig() {
 ...
 */
export function loadSandboxSections(workspaceRoot) {
    migrateUserConfig(heimdallConfigDir());
    ensureGeneratedDefaultConfig();
    if (workspaceRoot)
        migrateWorkspaceConfig(workspaceRoot);
    const sectionOf = (dir, basename) => {

...
function registerTools(ctx, config) {
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            await runWtOk(ctx, createArgs(config.bin, { branch: args.branch, base: args.base, hooks: args.hooks }), cwd, exec.signal);
            const entry = await findBranch(ctx, config, cwd, args.branch, exec.signal);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(_args, exec) {
            const cwd = sessionCwd(exec.agent);
            const entries = await listWorktrees(ctx, config.bin, cwd, exec.signal);
            return {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            await runWtOk(ctx, removeArgs(config.bin, { branch: args.branch, force: args.force, forceDeleteBranch: args.forceDeleteBranch, keepBranch: args.keepBranch }), cwd, exec.signal);
            if (entry !== undefined)
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const entry = await findBranch(ctx, config, cwd, args.branch, exec.signal);
            if (entry === undefined)
                throw new WtError('NOT_FOUND', `no worktree for branch ${JSON.stringify(args.branch)} — check \`worktrunk_list\`.`);
            if (args.keepWorktree !== true)
                assertNotSessionWorktree(entry, cwd, 'merge (it removes the worktree)');
            try {
                await runWtOk(ctx, mergeArgs(config.bin, { target: args.target, keepCommit: args.keepCommit, keepWorktree: args.keepWorktree }), entry.path, exec.signal);
            }
            catch (error) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            const cwd = sessionCwd(exec.agent);
            await runWtOk(ctx, copyIgnoredArgs(config.bin, { force: args.force, requireInclude: args.requireInclude }), cwd, exec.signal);
            return { ok: true };
        },
...
async function executeWtCommand(ctx, config, invocation) {
    ...
    try {
        ...
        switch (parsed.kind) {
            ...
            case 'list': {
                const entries = await listWorktrees(ctx, config.bin, cwd, invocation.signal);
            ...
            case 'merge': {
                const entry = await findBranch(ctx, config, cwd, parsed.branch, invocation.signal);
                if (entry === undefined)
                    return { kind: 'error', text: `no worktree for branch ${JSON.stringify(parsed.branch)} — check /wt list.` };
                if (parsed.keepWorktree !== true)
                    assertNotSessionWorktree(entry, cwd, 'merge (it removes the worktree)');
                try {
                    await runWtOk(ctx, mergeArgs(config.bin, { target: parsed.target, keepCommit: parsed.keepCommit, keepWorktree: parsed.keepWorktree }), entry.path, invocation.signal);
                }
                catch (error) {
                    if (error instanceof WtError && error.code === 'WT_FAILED') {
            ...
            case 'remove': {
                const entry = await findBranch(ctx, config, cwd, parsed.branch, invocation.signal);
                assertNotSessionWorktree(entry, cwd, 'remove');
                await runWtOk(ctx, removeArgs(config.bin, { branch: parsed.branch, force: parsed.force }), cwd, invocation.signal);
                if (entry !== undefined)
                    await unregisterWorkspace(ctx, entry.path);
                return { kind: 'success', text: `Removed worktree for ${JSON.stringify(parsed.branch)}.` };
            }
            case 'copy-ignored': {
                await runWtOk(ctx, copyIgnoredArgs(config.bin, {}), cwd, invocation.signal);
 ...
 */
function registerContextNote(ctx, config) {
    ...
    ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
        ...
        if (found === null || announced.has(sessionId))
            return decision;

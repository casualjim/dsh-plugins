...
export type WorktreePermissionStatus = 'applied' | 'already-full-access' | 'user-restricted' | 'unavailable';
/** Every stable failure the browser may branch on. */
export type WorktreeErrorCode = 'WT_NOT_INSTALLED' | 'NOT_A_REPO' | 'NO_INITIAL_COMMIT' | 'NO_LOCAL_BRANCH' | 'WT_FAILED' | 'WT_BAD_JSON' | 'WT_BUSY' | 'SESSION_WORKTREE' | 'PRESET_UNAVAILABLE' | 'PERMISSION_UNVERIFIED' | 'HOOK_FAILED' | 'NOT_FOUND';
export declare const WORKTREE_ERROR_CODES: readonly WorktreeErrorCode[];
/** JSON-safe value: the only data shape Typert lets cross the Remote boundary. */
export type WorktrunkJsonValue = string | number | boolean | null | readonly WorktrunkJsonValue[] | {
...
export type WorktrunkRemoteResult<Value> = {
    ...
    readonly ok: false;
    readonly error: WorktrunkFailure;
};
/** Build a failure value. */
export declare function createWorktrunkFailure(code: WorktreeErrorCode, message: string, details?: Readonly<Record<string, WorktrunkJsonValue>>): WorktrunkFailure;

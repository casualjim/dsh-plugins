...
export function withCompressedContent(message, text) {
...
export class HeadroomCompressor extends Service {
    ...
    constructor(ctx, config = {}) {
        ...
        this.ctx.effect(function* () {
            ...
            if (tools !== undefined)
                yield tools.register(buildRetrieveTool(self));
    ...
    applySettings(next) {
        ...
        if (transportChanged) {
            ...
            this.ctx.logger.info(`dsh-headroom proxy ${resolved.baseUrl ?? 'unset (degraded)'}`
     ...
     */
    async considerPass(session, signal) {
        if (!this.enabled)
            return null;
        if (this.processing)
            return null;
        const now = Date.now();
        if (now - this.lastCompressionTime < this.config.throttleMs)
            return null;
        ...
        if (tokens < this.config.minContextTokens)
            return null;
     ...
     */
    async compressSession(session, signal) {
        if (this._transport === null)
            return null;
        const nodes = snapshotSurface(session);
        const payload = buildCompressionPayload(nodes.map((node) => node.message), this.config.minMessageChars, this.config.renameToolCalls);
        ...
        if (inputFingerprint === this.lastOutputFingerprint)
            return null;
        // Input unchanged since the last attempt — nothing new to do.
        if (inputFingerprint === this.lastInputFingerprint)
            return null;
        const candidateFingerprint = generateCandidateFingerprint(payload);
        if (candidateFingerprint === this.lastGuardSkipCandidateFingerprint)
            return null;
        ...
        if (this.seen.hasAll(candidateHashes))
            return null;
        this.seen.ignoreSeen(payload);
        if (payload.mappings.every((mapping) => mapping.applyTo === null))
            return null;
        ...
        try {
            ...
            for (const change of applied.changes) {
                ...
                if (node === undefined || node.event.type !== 'tool/result')
                    continue;
    ...
    recordSkip(reason, inputFingerprint, candidateFingerprint) {
...
export function buildHeadroomCommand(service) {
    return {
        name: 'headroom',
        ...
        handler: (invocation) => handleCommand(service, invocation),
    };
}
/** Dispatch one `/headroom` invocation. */
async function handleCommand(service, invocation) {
    const parts = invocation.rawInput.trim().split(/\s+/).filter(Boolean);
    const first = (parts[0] ?? 'status').toLowerCase();
    if (first === 'mode')
        return handleModeChange(service, parts[1]?.toLowerCase() ?? '');
    switch (first) {
        case 'on': {
            service.toggle(true);
...
async function handleModeChange(service, rawMode) {
...
async function handleHealth(service) {
...
async function handleStats(service) {
    let proxy;
    if (service.transport !== null) {
        try {
...
async function handleRun(service, invocation) {
    if (!service.isEnabled())
        return { kind: 'error', text: 'Headroom is disabled. Run /headroom on first.' };
...
function renderStatus(service) {
    ...
    if (service.stats.lastError !== undefined)
        lines.push(`Last error: ${service.stats.lastError}`);
    else if (service.stats.lastSkipReason !== undefined)
        lines.push(`Last skip: ${service.stats.lastSkipReason}`);
...
export function buildRetrieveTool(service) {
    return {

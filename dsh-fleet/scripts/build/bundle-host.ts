...
// ponytail: local layout — scripts live inside the package, root is 3 levels up
...
if (!pkgDir.startsWith(ROOT + sep) || pkgDir === ROOT) {
  console.error(`[bundle-host] plugin dir must live under repo root (got: ${process.argv[2]})`)
  process.exit(1)
}
...
// 可选 external（按包，dsh.bundle.hostExternal）：native/平台包（.node）不能被
// esbuild 内联，保持 import 由 node_modules 解析。
const hostExternal = Array.isArray(pkgJson?.dsh?.bundle?.hostExternal) ? pkgJson.dsh.bundle.hostExternal : []
try {
  await build({
...
// 2b. shared 声明副本进包 — 本仓库 shared/ 已 vendor 进包内，跳过（hub 布局兼容保留）
if (existsSync(join(ROOT, 'shared')) && !existsSync(join(pkgDir, 'shared'))) {
...
// 3. LICENSE 进包（本仓库 LICENSE 在包内，非仓库根）
if (!existsSync(join(pkgDir, 'LICENSE')) && existsSync(join(ROOT, 'LICENSE'))) {

...
describe('Cavekit skill files', () => {
  it('includes the Cavekit skill surface', async () => {
  ...
  it('soft-uses cavecrew-investigator without requiring dsh-caveman', async () => {
    ...
    for (const markdown of [spec, check, backprop]) {
      ...
      expect(markdown).not.toContain('pi-caveman')
      expect(markdown).not.toContain('codebase-memory')
    }
  })

...
describe('Cavekit reference resources', () => {
  ...
  it('includes README, license, and built package entrypoint files', async () => {
    expect((await stat(path.join(root, 'README.md'))).isFile()).toBe(true)
    expect((await stat(path.join(root, 'LICENSE'))).isFile()).toBe(true)
    expect((await stat(path.join(root, 'lib/index.js'))).isFile()).toBe(true)
    ...
    expect(readme).toContain('FORMAT.md')
  })
})
...
describe('non-Pi and superseded resource exclusion', () => {
  ...
  it('does not package upstream installer, hook, plugin, extension, or managed config files', async () => {

...
// ---- normalizeToolName ----
{
  ...
  assert.throws(() => normalizeToolName("ctx", "mcp__other__tool"), /looks like the registered full name/);
}
...
// ---- scoreTool / searchCatalog ----
{
  const unit = {
    ...
    catalog: new Map([
      ...
      ["down", { discoveredAt: 0, tools: new Map(), unavailable: "connect failed" }],
    ]),
  ...
  assert.match(unavailable[0].reason, /connect failed/);
  // 空查询 → 能力摘要
...
// ---- callTool：路由一致性 / 未连接 / 策略 ----
{
  ...
  await assert.rejects(
    ...
    /is not in the current workspace|is not activated/,
  );
  // 未知 server 形态
  await assert.rejects(() => mw.callTool("ctx", "use_ctx", {}, undefined), /expected format/);
  // 未连接（enabled:false 不触发连接）→ 错误含下一步提示（ws_mcp_search 或 ws_mcp_list）
  await assert.rejects(
    () => mw.callTool(fullServerName(ROOT, "ctx"), "use_ctx", {}, undefined),
    /is disconnected or connect failed/,
  );
  ...
  await assert.rejects(
    () => mw.callTool(fullServerName(ROOT, "ctx"), "use_ctx", {}, undefined),
    /disabled by the user; reconnect it in the GUI MCP panel/,
  );
...
// ---- listCatalog：完整清单 / 过滤 / 空返回 / 截断 / unavailable / disabled ----
{
  ...
  assert.equal(downEntry.unavailable, "connect failed", "discovery failure carries reason");
  ...
  assert.throws(() => listCatalog(units, [ROOT], "@/other/root/ctx", 50, "project", "empty"), /is not in the current workspace/);
...
// ---- findToolDetail：精确命中 / 完整 schema / 错误三分 / tool 归一化 ----
{
  ...
  // 跨 server 前缀 → looks like the registered full name
  assert.throws(() => findToolDetail(units, ROOT, fullServerName(ROOT, "ctx"), "mcp__other__tool"), /looks like the registered full name/);
  // 错误三分 1：server 发现失败（附原因）
  assert.throws(
    () => findToolDetail(units, ROOT, fullServerName(ROOT, "down"), "x"),
    /discovery failed.*connect failed/,
  );
  // 错误三分 2：服务器未发现
  assert.throws(
    () => findToolDetail(units, ROOT, fullServerName(ROOT, "nope"), "x"),
    /not connected or not discovered/,
  );
  // 错误三分 2b：单元未激活
  assert.throws(

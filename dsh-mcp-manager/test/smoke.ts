...
const main = async () => {
  ...
  await checkAsync("中间层工具注册（ws_mcp_search / ws_mcp_call / ws_mcp_list / ws_mcp_detail + 路由一致性）", async () => {
    ...
    assert.match(registered.find((d) => d.name === "ws_mcp_detail").description, /No keyword search/, "detail 描述写明不做什么");
    ...
    await assert.rejects(() => searchDef.execute({}, { agent: undefined }), /cannot determine workspace/);
    await assert.rejects(() => listDef.execute({}, { agent: undefined }), /cannot determine workspace/);
    await assert.rejects(() => detailDef.execute({}, { agent: undefined }), /cannot determine workspace/);
    // 路由：有 agent 但 cwd 无项目 → 显式失败（ws_mcp_call 缺 server 参数先报必填）
    await assert.rejects(() => callDef.execute({}, { agent: { session: { header: { cwd: "/other" } } } }), /cannot determine workspace|both server and tool are required/);
    ...
    assert.match(emptyList.message, /No available MCP servers|No project-level MCP config/, "空返回明确提示");
    // detail 必填校验
    await assert.rejects(() => detailDef.execute({}, { agent: { session: { header: { cwd: "/proj" } } } }), /both server and tool are required/);
    // server 全名解析往返
  ...
  await checkAsync("中间层 all 模式：@global 覆盖（list/search 可见全局，call 放行 @global）", async () => {
    ...
    await assert.rejects(
      ...
      /is disconnected or connect failed|is not ready|connect failed|disabled by user/,
    );
    // call：非当前 root 的项目 root 仍拒绝（防跨空间串台）。
    await assert.rejects(
      () => callDef.execute({ server: fullServerName("/other", "ctx"), tool: "use_ctx" }, { agent }),
      /is not in the current workspace/,
    );
  ...
  await checkAsync("#362 A2：project 模式 detail/call 传全局级服务器 → 引导 mcp__ 直呼（不再谎报不属于工作空间）", async () => {
    ...
    await assert.rejects(
      ...
      /Global-scope servers|mcp__gctx__/,
  ...
  await checkAsync("#362 A1：ws_mcp_list 带 serverFilter 过滤 0 命中 → message 可归因（不谎报未配置）", async () => {
    ...
    assert.match(out.message, /No project-level servers match server="nope"/, "A1：message 归因到过滤条件");
    assert.match(out.message, /Visible project-level servers: ctx/, "A1：列出可见项目级服务器");
    assert.match(out.message, /Global-scope servers are not listed here/, "A1：提示全局级不列出");
  ...
  await checkAsync("#362 P0-1：工具级禁用三入口一致（callTool / pre-execute guard / mcp__ 直呼）", async () => {
    ...
    assert.match(deny.reason, /was disabled by the user in the MCP panel/, "拒绝原因含禁用语义声明");
    ...
    assert.equal(toolDisabledReason(fullServerName("/proj", "ctx"), "use_ctx").includes("mcp__-prefixed tools"), true, "禁用原因声明只作用于 mcp__ 前缀");
    ...
    await assert.rejects(
      ...
      /was disabled by the user in the MCP panel/,
  ...
  check("renderMcpCatalogMessage 结构与声明", () => {
    ...
    const msg = renderMcpCatalogMessage([{ name: "code-graph", text: "Code graph", scope: "project" }]);
    assert.equal(msg.role, "user");
    assert.equal(msg.source.kind, "mcp-catalog");
    assert.equal(msg.content[0].type, "text");
    assert.match(msg.content[0].text, /available_mcp_servers/);
    assert.match(msg.content[0].text, /not live connection state/);
    assert.match(msg.content[0].text, /ws_mcp_search/, "#228 目录文案引导经中间层调用");
    assert.match(msg.content[0].text, /`code-graph`: Code graph/);
    ...
    const mixed = renderMcpCatalogMessage([{ name: "bare-x" }, { name: "code-graph", text: "Code graph" }]);
    assert.match(mixed.content[0].text, /^- `bare-x`$/m, "双缺省行仅名字");
    assert.doesNotMatch(mixed.content[0].text, /`bare-x`: /);
    assert.match(mixed.content[0].text, /^- `code-graph`: Code graph$/m, "带描述行保持");
  });
  ...
  check("buildToolDefinition：超时下探与截断（L3）", async () => {
    ...
    assert.match(rendered[0].text, /truncated/, "长结果截断标注");
  });
  ...
  check("resolveCatalogInjection：history-based 去重（核心：多轮不重复注入）", () => {
    const supervisors = new Map([["code-graph", { server: { description: "Code graph" }, tools: [], toolMeta: new Map() }]]);
  ...
  check("resolveCatalogInjection：描述/缓存变化不注入、集合变化注入更新消息", () => {
    const base = new Map([["code-graph", { server: { description: "Code graph" }, tools: [], toolMeta: new Map() }]]);
    const agent = makeAgent();
    ...
    const added = new Map([
      ["code-graph", { server: { description: "Code graph" }, tools: [], toolMeta: new Map() }],
    ...
    assert.match(afterAdd[1].data.content[0].text, /replaces all previous/);

...

{
  ...
  assert.match(warned, /ctx.inject unavailable/, "ctx.inject 不可用应 warn");
}
...

{
  ...
  assert.match(warned, /lacks register/, "settings 服务缺失应 warn");
}
...

{
  ...
  installSettingsNamespace(ctx, "test-ns", {}, {}, { setSource: () => {}, onChange: () => {} });
  assert.match(warned, /settings.register failed/, "register 抛错应 warn");
}

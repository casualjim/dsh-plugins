...
const UiConfigSchema = z.object({
  ...
  zIndexBase: z.number().default(DEFAULT_UI_CONFIG.zIndexBase)
    .description("Floating panel z-index base (1-9000); the pill and the expanded main panel use the same value"),
});
...
export const Config: z<{
  ...
  enabled: z.boolean().default(true).description("Whether to enable this plugin"),
  announceToAgent: z.boolean().default(true).description("Whether to announce the plugin to the agent (capability list handled by <available_mcp_servers>)"),
  storePath: z.string().description("Global server config path; leave empty for the default <DSH_HOME>/dsh-mcp.json").disabled(true),
  announceCatalog: z.boolean().default(DEFAULT_ANNOUNCE_CATALOG).description("Whether to inject the MCP capability catalog (<available_mcp_servers>)"),
  catalogMaxEntries: z.number().default(DEFAULT_CATALOG_MAX_ENTRIES).description("Max entries injected into the catalog").disabled(true),
  enhanceEmptyDescriptions: z.boolean().default(DEFAULT_ENHANCE_EMPTY_DESCRIPTIONS).description("Append custom descriptions for tools with empty descriptions"),
  resultTruncateBytes: z.number().default(DEFAULT_RESULT_TRUNCATE_BYTES).description("Tool result truncation size in bytes").disabled(true),
  middleware: z.union([z.const("off"), z.const("project"), z.const("all")]).default("project")
    .description("MCP middleware mode: off = register mcp__ tools directly (default, compatible); project = project-level servers go through ws_mcp_search/ws_mcp_call (recommended); all = everything goes through middleware"),
  middlewarePolicy: z.dict(z.any()).default({})
    .description("Middleware policy: { allowTools: {<server>: [glob]}, denyTools: {<server>: [glob]} } where server is the bare name"),
  ui: UiConfigSchema,
});
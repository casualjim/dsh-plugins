...
export function normalizeToolName(serverName: string, toolName: string, caller = "ws_mcp_call"): string {
  ...
  if (name.startsWith("mcp__")) {
    ...
    if (name.startsWith("mcp__")) {
      throw new Error(
        `${caller}: tool argument looks like the registered full name of a different MCP server (${JSON.stringify(toolName)}，server="${serverName}"）; pass the bare tool name on that server`,
      );
    }
  }
  return name;
...
export function listCatalog(
  ...
  if (serverFilter !== undefined && serverFilter.startsWith("@")) {
    ...
    if (parsed === undefined || !roots.includes(parsed.root)) {
      throw new Error(
        `ws_mcp_list: server ${JSON.stringify(serverFilter)} is not in the current workspace (${JSON.stringify(roots)}); routing-consistency check failed (prevents cross-workspace bleed)`,
      );
    }

...
export function renderMcpCatalogMessage(entries: CatalogEntry[], mode?: string): CatalogMessage {
  ...
  const projectGuidance =
    "Project-level servers are always accessed through middleware tools: first search the workspace MCP tools with `ws_mcp_search`, then invoke with `ws_mcp_call` (server/tool come from the search results; for a full inventory of all servers and tools use `ws_mcp_list`, for a single tool's full inputSchema use `ws_mcp_detail`). **Project-level servers are never called via their mcp__-prefixed tools directly**.";
  const globalGuidance =
    mode === "all"
      ? "Global servers also go through middleware in all mode (global servers register no mcp__ tools in all mode): first search with `ws_mcp_search`, then invoke via `ws_mcp_call` - the same `ws_mcp_*` tools as project-level."
      : "Global servers are still called directly via `mcp__<server>__<tool>` prefixed tools (in project mode, global servers are not searchable through middleware).";
  const guidance = hasProject
    ? `${projectGuidance}${globalGuidance}`
    : mode === "all"
      ? globalGuidance
      : `When a task matches a server's capabilities, call its \`mcp__<server>__<tool>\` tools directly (exact tool names and parameters are in the tool list).${globalGuidance}`;
  const lines = [
    "<system-reminder>",

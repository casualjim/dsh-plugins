  ...
  async callTool(
    ...
    try {
      const result = await withTimeout(
        ...
        `ws_mcp_call: call timed out (${CALL_TIMEOUT_MS}ms); retry allowed. On repeated timeouts, verify parameters with ws_mcp_detail or check the server's status`,
        signal,
      );
      ...
      throw new Error(this.hostRedact(`ws_mcp_call: ${JSON.stringify(`${parsed.server}/${tool}`)} call failed: ${msgOf(error)}; verify the parameter schema with ws_mcp_detail, or confirm the server is connected and retry`));
    }
  }

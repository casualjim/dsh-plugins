...
At the end of the run, before handing back, check the log told the truth. Read `the current session transcript` for this run. For an earlier same-project session, use `session_search`; do not glob DSH's global session directory. Walk the log against what actually happened:

...
Before spawning investigators, list the available MCP tools in the current environment. Use the available-tools map when present; otherwise inspect the environment's MCP configuration for enabled MCP servers.
...
Spawn each investigator as a `poteto_agent` call:
- use a normal subagent. **Do not restrict its tool set.** Restricting tools strips MCP access, which disables MCP-backed investigators entirely. The source control investigator would be safe read-only, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox — the prompt forbids file writes.
...
Spawn one synthesizer `poteto_agent` subagent:
- use a normal subagent. The synthesizer's quality check spot-verifies citations, which can require MCP access. Tool availability varies by deployment configuration and defeats that.

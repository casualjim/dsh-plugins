...
The active harness session file is available to child commands as `the current session transcript`. To inspect earlier sessions for this same working directory, call `session_search` with `action: "list"`; it uses DSH's session API rather than guessing a path or scanning another project.

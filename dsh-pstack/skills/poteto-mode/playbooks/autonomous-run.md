...
**You own the exit condition. Define done, then drive to it without stopping.** For "going to bed" / "run until done" / "watch until X".
...
2. Pick the wake mechanism using harness sessions and background bash jobs (a built-in, not a pstack skill). An event to watch (CI, a merge, a ref advancing) gets a background polling job whose output you collect with `job_output`, with a long interval as fallback heartbeat. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.

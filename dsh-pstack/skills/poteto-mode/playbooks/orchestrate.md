...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned and drained only through the `subagent` tool. DSH subagents are fresh processes; do not assume they can be resumed. State reads and writes go through the store files directly at drain points, one command in and one line out, to conserve context. Nothing spawns, waits, or wakes outside this loop.
...
- **Worker / verifier.** Always a subagent unless the task needs this machine: available project UI or CLI runtime-verification tooling; reading the current session transcript or a `session_search` result; simulators and local IDE state; auth that exists only here. Subagents share the local machine but have isolated context, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier as a separate subagent from its worker.
...
Create `orchestrate/<project-slug>/` in the current workspace (path in the system prompt). Every file has exactly one writer; owners publish facts, readers aggregate at read time. Keep bookkeeping in plain TSV and JSON, readable without a CLI.
...
2. **Install the runtime.** Create the store files. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs by reading the host's PR list (`tea pr list` on Gitea, `gh pr list` on GitHub) plus the local branch tips.
...
7. **Close.** Drain the final inbox, reconcile every spawned agent to a terminal row (done, abandoned, zombie-reconciled), confirm the predicate on the real artifact, confirm every landed PR has a verdict for its current head SHA, audit the trail per show-me-your-work including its cross-reviewer review, encode recurring corrections into `preferences.md` or the brief template. Leave the store intact; it is the postmortem.

#### Queue and drain

- On a completion notification, append an inbox row `<agent> <unit> <status> [--report PATH]` and return to what you were doing. Never deep-review inline; a completion that needs review becomes a verifier unit. Never review a diff inside a drain.
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier polling-job wake (arm it as a background bash job, with a long interval fallback), and before a human report. Begin each batch by reading the inbox. Arrivals during a drain wait for the next one.
...
- A drain turn ends with the three status lines: counts against the states, what changed, gates open. Detail lives in `status.md`; the full reply contract applies at checkpoints and close.

#### Stack safety

- The frontier is a computed object, never narrative. Recompute `frontier.json` from the host PR list plus `git` branch tips after every merge and stack mutation, because PR base refs drift mid-rebase while the local branch graph is authoritative: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Resolve it in the stacker's clone; a checkout that never saw the stack reports no PRs and the computation errors rather than guessing.
- Exactly one stacker per stack may mutate stack topology, serialized within its stack; record the holder in the standing orders. Rebases run as dedicated units.
- Workers never rebase and never mutate topology. Babysitters follow `playbooks/babysit.md`, one per stack, scoped to one immutable frontier generation; they report conflicts to the stacker rather than rebasing.
...
- One retro polling job follows merged PRs for reverts, post-merge CI breaks, and orphaned follow-ups.

#### Verification

Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts; a dedicated verifier agent (a separate subagent from the worker) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.

Write ledger rows into `ledger.tsv` directly. Check the current PR and head SHA by re-reading `ledger.tsv`. `ledger.tsv`, one row per verdict, keyed by PR number plus head SHA: `live-ui-verified | unit-test-verified | type-check-only | verifier-blocked | verifier-failed`. CI green is an input to a verdict, not a verdict. Behavioral work needs better than `type-check-only`. `verifier-blocked` is not a pass; respawn when the environment heals. `verifier-failed` gets a fix unit, not a re-verify. A worker may self-report; a verifier overrides it on the same key. A new head SHA voids the row, so re-verify after restack. The ledger answers "was this verified", not memory and not the transcript.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the child.s status in the available process or repository evidence. Transcript mtime is not liveness.
...
- Retry by mode: cap-hit or oom, respawn with smaller scope; network-drop, retry as-is; tool-error, retry as-is; unknown, retry once. Two retries, then abandon the unit and replan around it.

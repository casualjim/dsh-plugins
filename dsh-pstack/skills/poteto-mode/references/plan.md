...
- Prefer the `poteto_agent` tool. Use the generic `subagent` tool when a specialized prompt is needed.
- Children inherit the parent model; there is no per-call model routing.
...
If a phase creates or edits a skill, the phase instructs the implementer to use the DSH SKILL.md format for `SKILL.md` authoring.

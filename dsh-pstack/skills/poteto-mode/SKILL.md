...
- Any prose surface → the **unslop** skill. Your reply is a prose surface. Agent-facing skill prose follows the DSH SKILL.md format.
...
**Always obtain explicit confirmation** before external or irreversible actions, including force-pushes, PR creation or updates, CI triggers, merges, deployments, data deletion, and customer messages. The harness sandbox and approval stack gate these actions; never bypass or work around a denial.
...
**Use the `poteto_agent` tool (bundled `poteto-agent` persona) for implementation delegates and ad-hoc helpers.** The bundled agent reads this skill in full before it works. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) may use the generic `subagent` tool or other agents to preserve independent review.

**Defaults for each delegation.** Keep the task self-contained, point at files instead of inlining large payloads. DSH's subagent tools take a short description and one complete prompt; fan out with several parallel calls when the work is independent. Spawned children are fresh conversations without the parent's context, so each brief names every file, fact, and constraint the child needs. Children inherit the current model selection. Run in the background when the next step does not depend on the result.

You own every subagent's work. Review the diff and write your own summary, don't pass through what it said. Interrupting a child stops only its current turn and parks queued follow-ups, so fire a fresh subagent with consolidated scope rather than trusting a "done" summary. A second opinion is the same prompt against a different model. Agreement is high-signal.
...
- **Shipping.** The half after Babysit. Independently verifying a green stack, then landing the contiguous verified run bottom-up through the host's PR tooling (`tea` / `gh`). `playbooks/shipping.md`.

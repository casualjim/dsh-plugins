...
export function parsePersona(file: string): Persona {
  ...
  if (!name) throw new Error(`missing name in ${file}`)
  if (tools === undefined) throw new Error(`missing tools in ${file}`)
...
function alwaysOnSectionText(defaultMode: string): string {
  return [
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  if (config.enableCavecrew !== false) {
    ...
    for (const entry of readdirSync(agentsDir, { withFileTypes: true })) {
      if (!entry.isFile() || !entry.name.startsWith('cavecrew-') || !entry.name.endsWith('.md')) continue

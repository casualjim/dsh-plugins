...
export const DISCOVERY_GATE_NOTICE = 'codebase-memory reminder: for broad code discovery, prefer the codebase-memory MCP tools ' +
 ...
 */
export function extractSearchToken(value) {
...
export function gateToken(toolName, args) {
    if (!RAW_DISCOVERY_TOOLS.has(toolName) || typeof args !== 'object' || args === null)
        return undefined;
    const record = args;
...
export function formatAugmentation(token, results) {
...
async function searchGraphOnce(bin, project, token, budgetMs, signal) {
    ...
    try {
        ...
        if (NOT_INDEXED.test(message))
            return 'project-error';
        return undefined;
    }
}
function parseSearchGraphOutput(text) {
 ...
 */
export async function buildGraphAugmentation(bin, cwd, token, budgetMs, signal) {

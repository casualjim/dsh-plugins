...
// Type-only side effects: these packages declaration-merge the lifecycle and
// tool-pipeline events onto `@deepseek-ai/cordis` Events, keeping every
// ctx.on() below fully typed.
...
export function parseSkillFile(file: string): Pick<SkillRegistration, 'name' | 'description' | 'content'> {
	const raw = readFileSync(file, 'utf8')
	const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?/.exec(raw)
	if (match === null) throw new Error(`missing frontmatter in ${file}`)
	...
	if (!meta.name || !meta.description) throw new Error(`missing name/description in ${file}`)
...
const TIERS: readonly GraphTier[] = ['scout', 'verify', 'auditor']
...
export function apply(ctx: Context, config: Config = {}): void {
	...
	if (config.enableAugment !== false) {
		ctx.on('tools/post-execute', async (exec, _result, next) => {
			...
			if (!RAW_DISCOVERY_TOOLS.has(exec.name)) return downstream
			const contexts = []
			if (!advised) {
				advised = true
			...
			if (token !== undefined && exec.signal.aborted === false) {
				...
				if (cwd !== undefined) {
					try {
						...
						if (augmentation !== undefined) contexts.push(textMessage(augmentation))
					} catch {

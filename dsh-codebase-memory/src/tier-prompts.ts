...
export const BASE_TOOLS = ['read', 'grep', 'glob'] as const

const SHARED_DIRECT_CONTRACT =
...
const DIRECT: Record<string, string> = {
	scout:
		'Tier 1 — Scout. Perform positive, provisional discovery with about 3-4 narrow graph ' +
	...
	verify:
		'Tier 2 — Verify is the default tier. Gather task-directed evidence with narrow search, ' +
...
const HANDOFF: Record<string, string> = {
	...
	verify:
		'Tier 2 — Verify handoff is default. Cross-check supplied graph findings and coverage ' +
		'alerts against exact source, identify precisely what the parent must query instead of ' +
		'guessing.',
	auditor:
		'Tier 3 — Auditor handoff. Require bounded scope, current generation, complete relevant ' +
...
export function tierPersona(tier: GraphTier, access: GraphAccess): string {
	return (access === 'handoff' ? HANDOFF : DIRECT)[tier]
}

/** Model-facing delegation tool names, one per tier. */
export function tierToolName(tier: GraphTier): string {
	return tier === 'verify' ? 'codebase_memory' : `codebase_memory_${tier}`
}

export function tierToolFilter(tier: GraphTier, access: GraphAccess): { readonly allow: string[] } {
